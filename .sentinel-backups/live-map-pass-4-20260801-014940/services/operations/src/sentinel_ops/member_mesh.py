...
def member_map_config():
    """Return browser-safe map configuration.

    Browser map keys must be restricted to the dashboard origin in the provider
    console. The app uses Google photorealistic 3D/Street View when configured,
    and keeps the existing MapLibre renderer as a no-key fallback.
    """
    immersive = bool(GOOGLE_MAPS_API_KEY)
    return {
        "engine": "google-3d" if immersive else "maplibre",
...
def geocode_address(q: str = Query(..., min_length=3)):
    # Prefer a rooftop-capable provider. The demo-address index is a final offline
    # fallback only; it must never silently override a manually corrected pin.
...
def update_member_camera_location(camera_id: str, body: CameraLocationUpdate):
    ...
    with connect() as db:
        db.execute(
            "UPDATE member_cameras SET latitude=?, longitude=?, last_seen_at=? WHERE camera_id=?",
            (body.latitude, body.longitude, _now(), camera_id),
        )
