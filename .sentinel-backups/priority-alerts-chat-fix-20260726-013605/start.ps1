...
if (-not $NoChat -and (Test-Path (Join-Path $chatDir "server.js"))) {
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        if (Test-Path (Join-Path $chatDir "node_modules\ws")) {
            $chatArgs = "/c set PORT=$ChatPort&& set HOST=127.0.0.1&& node server.js"
            $chatProcess = Start-Process -FilePath "cmd.exe" -ArgumentList $chatArgs -WorkingDirectory $chatDir -WindowStyle Hidden -PassThru
