...
def auto_ingest_case_camera_inbox(case_id: str):
    ...
    for item in uploads:
        ...
        upload_id = f"UPL-{case_id.replace('CASE-', '')[:10]}-{source_key[-6:]}"
        with connect() as db:
            existing = _row(db.execute(
        ...
        try:
            result = _process_case_plate_path(
                ...
                captured_at=incident_time + timedelta(seconds=int(item.get("captured_offset_seconds") or 0)),
                source_upload_id=upload_id,
                source_media_url=item.get("media_url"),
                only_plate_events=True,
            )
            with connect() as db:
                db.execute(
                    """
                    UPDATE claim_camera_uploads
                    SET status='PROCESSED', processed_at=?, event_count=?, plate_count=?, error_message=NULL
                    WHERE case_id=? AND source_key=?
                    """,
                    (_now(), result["event_count"], len(result["observations"]), case_id, source_key),
                )
                _activity(
                    db, case_id, "CAMERA_UPLOAD_PROCESSED", "Stored camera clip processed",
                    f"{item.get('display_name')}: {len(result['observations'])} readable plate observation(s) stored.",
                    actor="Camera AI",
                    payload={"upload_id": upload_id, **result},
                )
            ...
            processed.append({"upload_id": upload_id, "status": "PROCESSED", **result})
        except Exception as exc:
            with connect() as db:
                db.execute(
