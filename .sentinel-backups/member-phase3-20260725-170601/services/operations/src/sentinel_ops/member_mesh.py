...
# Three adjacent demo homes. The coordinates are deliberately close together so
# the movement trail is legible during an offline stage demo. Normal address
# entry still uses Nominatim when internet is available.
DEMO_USERS = [
    {
        "user_id": "USR-001",
        "display_name": "User 1",
...
def initialise_member_store() -> None:
    with connect() as db:
        db.executescript(
            ...
            CREATE INDEX IF NOT EXISTS idx_member_incidents_profile
                ON member_incidents(profile_id, status, started_at);
...
class IncidentWatchStart(BaseModel):
    sighting_id: str
    incident_type: str = "DEMO_INTRUSION"
...
async def detect_member_faces(image: UploadFile = File(...)):
    """Run the bundled local OpenCV detector on a browser frame.

    This is the no-CDN fallback used by the Member webcam. It keeps the live
    camera and green tracking box working even when TensorFlow/BlazeFace cannot
    be downloaded at the venue.
    """
...
def _incident_payload(db: sqlite3.Connection, incident_id: str) -> dict[str, Any] | None:
    ...
    for row in notices:
        ...
        item["distance_from_origin_m"] = round(
            _distance_metres(origin_lat, origin_lon, float(item["latitude"]), float(item["longitude"])),
            1,
        )
        items.append(item)
...
def _active_incident_for_profile(db: sqlite3.Connection, profile_id: str) -> dict[str, Any] | None:
    row = db.execute(
        """
        SELECT incident_id FROM member_incidents
...
async def create_face_sighting(
    ...
    with connect() as db:
        ...
        if active:
            ...
            db.execute(
                "UPDATE member_incidents SET updated_at=? WHERE incident_id=?",
                (captured_at, incident_id),
            )
            notification_id = f"NOTE-{uuid.uuid4().hex[:10].upper()}"
            db.execute(
                """
                INSERT INTO member_camera_notifications(
                    notification_id, incident_id, camera_id, user_id, status,
                    reason, created_at, updated_at, captured_sighting_id
                ) VALUES (?, ?, ?, ?, 'MATCH_CAPTURED', ?, ?, ?, ?)
                ON CONFLICT(incident_id, camera_id) DO UPDATE SET
                    status='MATCH_CAPTURED',
                    reason=excluded.reason,
                    updated_at=excluded.updated_at,
                    captured_sighting_id=excluded.captured_sighting_id
                """,
                (
...
def start_member_incident_watch(body: IncidentWatchStart):
    """Start a demo camera-mesh watch from a human-confirmed incident sighting.

    Facial similarity alone never starts an incident. The operator must confirm
    that an intrusion/theft event occurred, after which neighbouring cameras are
    placed in watch mode for the same anonymous profile.
    """
    ...
    with connect() as db:
        ...
        if existing:
            return {
                "created": False,
                "incident": _incident_payload(db, existing["incident_id"]),
            }
        ...
        db.execute(
            ...
            """,
            (
                incident_id, sighting["profile_id"], sighting["user_id"],
                sighting["camera_id"], sighting["sighting_id"],
                body.incident_type.strip().upper() or "DEMO_INTRUSION",
                started_at, started_at,
            ),
        )
        cameras = db.execute(
            "SELECT * FROM member_cameras ORDER BY user_id, camera_id"
        ).fetchall()
        for camera in cameras:
            is_origin = camera["camera_id"] == sighting["camera_id"]
            status = "ORIGIN_CONFIRMED" if is_origin else "WATCH_ACTIVE"
            reason = (
                f"Confirmed incident origin at {sighting['household']}"
                if is_origin
                else f"Neighbour watch activated from {sighting['household']}"
            )
            db.execute(
                """
                INSERT INTO member_camera_notifications(
                ...
                """,
                (
                    f"NOTE-{uuid.uuid4().hex[:10].upper()}", incident_id,
                    camera["camera_id"], camera["user_id"], status, reason,
                    started_at, started_at,
                    sighting["sighting_id"] if is_origin else None,
                ),
            )
        return {"created": True, "incident": _incident_payload(db, incident_id)}
...
def member_mesh_state():
    ...
    with connect() as db:
        cameras = [dict(row) for row in db.execute(
            """
            SELECT c.*, u.display_name FROM member_cameras c
            JOIN member_users u ON u.user_id=c.user_id
            ORDER BY c.user_id, c.camera_id
            """
        ).fetchall()]
        profile_rows = db.execute(
            "SELECT * FROM face_profiles ORDER BY last_seen DESC"
        ).fetchall()
        trails = []
    ...
    return {
        ...
        "storage": "SQLite local demo database",
    }
...
def reset_face_sightings():
    ...
    with connect() as db:
        counts = {
            ...
            "incidents": db.execute("SELECT COUNT(*) AS n FROM member_incidents").fetchone()["n"],
            "notifications": db.execute("SELECT COUNT(*) AS n FROM member_camera_notifications").fetchone()["n"],
        }
