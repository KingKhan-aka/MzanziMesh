...
async def create_face_sighting(
    ...
    with connect() as db:
        ...
        profile_system_status = (
            str(best["row"]["system_status"] or "UNKNOWN") if matched and best else "UNKNOWN"
        )
        ...
        if active:
            ...
            db.execute(
                ...
                WHERE incident_id=? AND camera_id<>?
                  AND status IN ('WATCH_ACTIVE', 'ORIGIN_CONFIRMED')
                """,
    ...
    if auto_incident_started and incident_watch:
        try:
            ...
            with connect() as security_db:
                security_dispatch = create_dispatch_for_member_incident(
                    ...
                    actor="MzansiMesh automatic watch",
                )
