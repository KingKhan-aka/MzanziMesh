...
def _embedding_from_image(image: np.ndarray) -> tuple[np.ndarray, float, np.ndarray, dict[str, int]]:
...
async def create_face_sighting(
    ...
    vector, server_confidence, crop, face_box = _embedding_from_image(frame)
...
def classify_visitor(profile_id: str, body: ProfileClassificationIn):
    ...
    return {"profile": payload, "incident": incident}
