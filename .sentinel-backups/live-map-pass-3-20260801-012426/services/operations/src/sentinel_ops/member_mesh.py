...
def member_map_config():
    """Return non-secret renderer configuration for the browser map.

    The renderer uses an inline style so a remote style-document failure cannot
    silently force the demo back to the old Leaflet heat bubbles. All camera and
    event coordinates still come from SQLite.
    """
    return {
        "engine": "maplibre",
...
def geocode_address(q: str = Query(..., min_length=3)):
    ...
    raise HTTPException(
        status_code=404,
        detail="Address could not be geocoded. Try the full street, suburb and city, or use a configured demo property.",
    )
