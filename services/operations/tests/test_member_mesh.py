...
def test_three_demo_members_and_persistent_cameras(tmp_path: Path, monkeypatch):
    ...
    assert cameras.json()["cameras"][0]["household"] == "10 Killarney Avenue"
...
def test_offline_demo_geocoder(tmp_path: Path, monkeypatch):
    ...
    response = client.get("/api/member/geocode", params={"q": "11 Killarney Avenue"})
    assert response.status_code == 200
    data = response.json()
    assert data["suburb"] == "Lakefield"
    assert data["latitude"] == -26.184499
    assert data["longitude"] == 28.28823
...
def test_local_detector_repeat_match_and_incident_watch(tmp_path: Path, monkeypatch):
    ...
    assert statuses["10 Killarney Avenue"] == "ORIGIN_CONFIRMED"
    assert statuses["11 Killarney Avenue"] == "WATCH_ACTIVE"
    assert statuses["12 Killarney Avenue"] == "WATCH_ACTIVE"

    # Keep this contract test focused on an actionable continuity candidate. The
    # production path correctly refuses automatic escalation when travel between
    # configured cameras is physically impossible.
    from sentinel_ops.storage import connect
    with connect() as db:
        origin = db.execute(
            "SELECT latitude, longitude FROM member_cameras WHERE camera_id='CAM-U1-01'"
        ).fetchone()
        db.execute(
            "UPDATE member_cameras SET latitude=?, longitude=?, pin_accuracy='EXACT_MANUAL' WHERE camera_id='CAM-U2-01'",
    ...
    assert status_by_house["11 Killarney Avenue"] == "MATCH_CAPTURED"
