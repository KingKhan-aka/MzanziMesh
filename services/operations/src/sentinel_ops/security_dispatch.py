...
def create_test_security_alert():
    ...
    The shortcut never invents an identity. It reuses an active Member incident,
    or starts a human-labelled demo watch from the latest stored camera sighting.
    """
    initialise_security_store()
    with connect() as db:
        active = db.execute(
    ...
    with connect() as db:
        ...
        if not sighting:
            raise HTTPException(
                status_code=404,
                detail="No camera sighting is available. Detect a person in My Property first.",
            )
...
def whatsapp_status():
    """Truthful capability response for the Abed security frontend.

    Abbasi's backend deliberately keeps outbound WhatsApp in a local review queue.
    Returning an explicit disabled state keeps the UI usable without pretending an
    external message was sent or requiring credentials in the hackathon package.
    """
    return {
        "configured": False,
        ...
        "delivery_mode": "LOCAL_REVIEW_QUEUE_ONLY",
    }
...
def send_notification_whatsapp(notification_id: str):
    ...
    with connect() as db:
        exists = db.execute(
            "SELECT 1 FROM security_notifications WHERE notification_id=?",
            (notification_id,),
        ).fetchone()
    if not exists:
    ...
    raise HTTPException(
        status_code=503,
        detail="WhatsApp Business is not configured. The reviewed notification remains safely queued locally.",
    )
